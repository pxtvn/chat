            ...
            switch (data.type) {
                ...
                case 'text':
                    if (ws.currentRoom) {
                        ...
                        // **SỬA LỖI TẠI ĐÂY**
                        // Gửi kèm cả ID tạm thời (`data.tempId`) mà client đã gửi lên
                        broadcastToRoom(ws.currentRoom, {
                            ...
                            tempId: data.tempId // Thêm dòng này
                        });
                    }
                    break;
...
server.listen(PORT, () => console.log(`Server is listening on port ${PORT}`));const http = require('http');
const express = require('express');
const WebSocket = require('ws');
const path = require('path');
const { v4: uuidv4 } = require('uuid');
